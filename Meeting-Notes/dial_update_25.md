...
Reads TDI and TDS files (600 characters per record)
Loads into Oracle RAW table
Uses Oracle export tool to create combo.raw file, sorted by TIN number
Sequential C code processing through combo.raw
Calculates entity attributes during the single-pass sequential read
...
ENT_TDI_XREF - Cross-reference flag calculated based on record order
ENT_TYPE - Entity type (1, 2, or 3) determined by which record processes first
LARGE_REPEAT - Flag depending on ordered processing
Aggregate values - Running cumulative sums (e.g., 100 → 199 → 299 → 349)
