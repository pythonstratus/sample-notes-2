 ...
 * Multiple datasources are supported for different database connections.
 * 
...
public class ComboFileGenerator implements Tasklet {
    ...
    // Core dependencies with proper injection
    private final JdbcTemplate primaryJdbcTemplate;
    private final JdbcTemplate dialJdbcTemplate;
    private final JdbcTemplate dialRptJdbcTemplate;
    private final JdbcTemplate alsJdbcTemplate;
    private final JdbcTemplate alsRptJdbcTemplate;
    ...
    /**
     * Constructs a new ComboFileGenerator with all required dependencies.
     * 
     * @param primaryDataSource Primary datasource for main operations
     * @param dialDataSource Datasource for DIAL schema operations
     * @param dialRptDataSource Datasource for DIAL reporting operations
     * @param alsDataSource Datasource for ALS schema operations
     * @param alsRptDataSource Datasource for ALS reporting operations
    ...
    public ComboFileGenerator(
            ...
            @Qualifier("dialRptDataSource") DataSource dialRptDataSource,
            @Qualifier("alsDataSource") DataSource alsDataSource,
            @Qualifier("alsRptDataSource") DataSource alsRptDataSource,
            DialEnvironmentConfig config,
            @Qualifier("dialExecutorService") ExecutorService executorService) {
        
        // Initialize JdbcTemplates for each datasource
        this.primaryJdbcTemplate = new JdbcTemplate(primaryDataSource);
        this.dialJdbcTemplate = new JdbcTemplate(dialDataSource);
        this.dialRptJdbcTemplate = new JdbcTemplate(dialRptDataSource);
        this.alsJdbcTemplate = new JdbcTemplate(alsDataSource);
        this.alsRptJdbcTemplate = new JdbcTemplate(alsRptDataSource);
    ...
    private boolean gatherTableStatsBatch(String schemaName, List<String> tableNames, BufferedWriter logWriter) throws IOException {
        return dialJdbcTemplate.execute(connection -> {
            boolean allSuccess = true;
    ...
    private void logTableStatsInfo(String schemaName, String tableName, BufferedWriter logWriter) throws IOException {
        ...
        try {
            dialJdbcTemplate.queryForObject(sql, (rs, rowNum) -> {
    ...
    private boolean performDialRptSync() {
        ...
        return executeSyncOperation("dialrpt", dialRptJdbcTemplate);
    }
    ...
    private boolean performAlsSync() {
        ...
        return executeSyncOperation("als", alsJdbcTemplate);
    }
    ...
    private boolean performAlsRptSync() {
        ...
        return executeSyncOperation("alsrpt", alsRptJdbcTemplate);
    }
    ...
    private boolean executeSyncOperation(String connectionName, JdbcTemplate jdbcTemplate) {
        ...
        try {
            ...
            // Execute @syn2cp procedure using JdbcTemplate
            boolean success = jdbcTemplate.execute(connection -> {
    ...
    public boolean executePreparedStatement(final String sql, final Object... params) {
        return primaryJdbcTemplate.execute(sql, new PreparedStatementCallback<Boolean>() {
            @Override
    ...
    public void validateConfiguration() {
        ...
        if (statsGatheringEnabled && dialJdbcTemplate == null) {
            throw new IllegalStateException("DIAL datasource is required for statistics gathering");
        }
