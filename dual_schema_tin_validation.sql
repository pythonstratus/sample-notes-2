...
BEGIN
    ...
    FOR rec IN (
        ...
        v_record := t_tin_section_data(
            ...
            RAWTOHEX(DBMS_CRYPTO.HASH(UTL_RAW.CAST_TO_RAW(v_fields), DBMS_CRYPTO.HASH_MD5)),
            v_data_line
        );
        PIPE ROW(v_record);
    END LOOP;
...
BEGIN
    ...
    -- *** CUSTOMIZE THIS SECTION FOR YOUR LEGACY SCHEMA ***
    -- Replace table names and column names with your actual legacy schema structure
    
    -- Legacy ENT Query (customize table and column names)
    v_sql := 'SELECT 
                LEGACY_TIN_COLUMN,           -- Replace with actual TIN column
                LEGACY_TINSID_COLUMN,        -- Replace with actual TINSID column  
                LEGACY_TPCTRL_COLUMN,        -- Replace with actual TPCTRL column
                LEGACY_TOTASSD_COLUMN,       -- Replace with actual TOTASSD column
                LEGACY_STATUS_COLUMN,        -- Replace with actual STATUS column
                LEGACY_CASEIND_COLUMN,       -- Replace with actual CASEIND column
                LEGACY_PYRIND_COLUMN,        -- Replace with actual PYRIND column
                LEGACY_CASECODE_COLUMN,      -- Replace with actual CASECODE column
                LEGACY_SUBCODE_COLUMN,       -- Replace with actual SUBCODE column
                LEGACY_ASSNCFF_COLUMN,       -- Replace with actual ASSNCFF column
                LEGACY_ASSNGRP_COLUMN,       -- Replace with actual ASSNGRP column
                LEGACY_RISK_COLUMN,          -- Replace with actual RISK column
                LEGACY_ARISK_COLUMN,         -- Replace with actual ARISK column
                LEGACY_EXTRDT_COLUMN         -- Replace with actual EXTRDT column
              FROM ' || p_legacy_schema || '.LEGACY_ENT_TABLE_NAME
              WHERE LEGACY_TIN_COLUMN = ' || v_tptin;
    ...
    -- *** ADD SIMILAR SECTIONS FOR OTHER TABLES ***
    -- Customize these queries for your legacy TRANTRAIL, ENTMOD, ENTACT, TIMETIN tables
    -- Follow the same pattern as above but with your legacy table/column names
    
    -- For now, return placeholder records to indicate customization needed
    v_record := t_tin_section_data(
        'LEGACY', 'CUSTOMIZATION_NEEDED', v_tptin, NULL, 
        'CUSTOMIZE_LEGACY_QUERIES', 'PENDING',
        'LEGACY SCHEMA QUERIES NEED CUSTOMIZATION - Update FN_GET_LEGACY_TIN_DATA function'
    );
    PIPE ROW(v_record);
